...
skip_unlock: __attribute__ ((unused));
	 ...
	 * our threshold.
	 *
	 * Note: We use 'capacity' rather than arch_scale_cpu_capacity() so that
	 * thermal throttling correctly lowers the threshold and triggers misfit
	 * migration sooner when the CPU cannot sustain its nominal load.
	 *
	 * Multiply by 1024 (the PELT period constant) rather than USEC_PER_MSEC
	 * (1000) for accurate period-to-microsecond conversion. Each iteration
	 * of approximate_runtime() advances by delta = 1024 PELT units ~ 1024 us.
	 *
	...
	limit = limit > TICK_USEC ? limit - TICK_USEC : 0;
